...
export class ProjectService {
  ...
  getProject(projectId: number): Observable<any> {
    return this.http
      .get(`/uai-harbor/api/projects/${projectId}`, HTTP_GET_OPTIONS)
      .pipe(
  ...
  listProjects(
    ...
    return this.http
      .get(`/uai-harbor/api/projects`, buildHttpRequestOptions(params))
      .pipe(
  ...
  toggleProjectPublic(projectId: number, isPublic: string): Observable<any> {
    return this.http
      .put(
        `/uai-harbor/api/projects/${projectId}`,
  ...
  deleteProject(projectId: number): Promise<any> {
    return this.http
      .delete(`/uai-harbor/api/projects/${projectId}`)
      .toPromise()
  ...
  checkProjectExists(projectName: string): Observable<any> {
    return this.http
      .head(`/uai-harbor/api/projects/?project_name=${projectName}`)
      .pipe(
        map(response => response.status),
        catchError(error => observableThrowError(error))
      );
  ...
  checkProjectMember(projectId: number): Observable<any> {
    return this.http
      .get(`/uai-harbor/api/projects/${projectId}/members`, HTTP_GET_OPTIONS)
      .pipe(
        map(response => response.json()),
        catchError(error => observableThrowError(error))
      );
