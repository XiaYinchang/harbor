...
export class MemberService {
  ...
  listMembers(projectId: number, entity_name: string): Observable<Member[]> {
    return this.http
      .get(
        `/uai-harbor/api/projects/${projectId}/members?entityname=${entity_name}`,
        HTTP_GET_OPTIONS
      )
      .pipe(
  ...
  addUserMember(
    ...
    return this.http
      .post(
        `/uai-harbor/api/projects/${projectId}/members`,
        {
          role_id: roleId,
  ...
  changeMemberRole(
    ...
    return this.http
      .put(
        `/uai-harbor/api/projects/${projectId}/members/${userId}`,
        { role_id: roleId },
        HTTP_JSON_OPTIONS
      )
      .toPromise()
  ...
  deleteMember(projectId: number, memberId: number): Promise<any> {
    return this.http
      .delete(`/uai-harbor/api/projects/${projectId}/members/${memberId}`)
      .toPromise()
      .then(response => response.status)
      .catch(error => Promise.reject(error));
  }
