...
const userGroupEndpoint = "/uai-harbor/api/usergroups";
const ldapGroupSearchEndpoint = "/uai-harbor/api/ldap/groups/search?groupname=";
...
export class GroupService {
  ...
  createGroup(group: UserGroup): Observable<any> {
    return this.http.post(userGroupEndpoint, group, HTTP_JSON_OPTIONS).pipe(
      map(response => {
        return this.extractData(response);
      }),
  ...
  deleteGroup(group_id: number): Observable<any> {
    return this.http.delete(`${userGroupEndpoint}/${group_id}`).pipe(
      map(response => {
        return this.extractData(response);
      }),
      catchError(this.handleErrorObservable)
