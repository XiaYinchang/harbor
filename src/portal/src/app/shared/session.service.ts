...
const currentUserEndpoint = "/uai-harbor/api/users/current";
const signOffEndpoint = "/uai-harbor/c/log_out";
const accountEndpoint = "/uai-harbor/api/users/:id";
const langEndpoint = "/language";
const userExistsEndpoint = "/uai-harbor/c/userExists";
const renameAdminEndpoint = "/uai-harbor/api/internal/renameadmin";
const langMap = {
  zh: "zh-CN",
  en: "en-US"
};
...
export class SessionService {
  ...
  signIn(signInCredential: SignInCredential): Promise<any> {
    ...
    // Trigger Http
    return this.http
      .post(signInUrl, queryParam, HTTP_FORM_OPTIONS)
      .toPromise()
      .then(res => {
        const token = res.headers.get("keystone-token");
  ...
  retrieveUser(): Promise<SessionUser> {
    return this.http
      .get(currentUserEndpoint, HTTP_GET_OPTIONS)
      .toPromise()
  ...
  signOff(): Promise<any> {
    return this.http
      .get(signOffEndpoint, HTTP_GET_OPTIONS)
      .toPromise()
      .then(() => {
