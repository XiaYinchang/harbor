...
const passwordChangeEndpoint = "/uai-harbor/api/users/:user_id/password";
const sendEmailEndpoint = "/uai-harbor/c/sendEmail";
const resetPasswordEndpoint = "/uai-harbor/c/reset";
...
export class PasswordSettingService {
  ...
  changePassword(userId: number, setting: PasswordSetting): Promise<any> {
    if (
      !setting ||
      setting.new_password.trim() === "" ||
      setting.old_password.trim() === ""
    ) {
    ...
    return this.http
      .put(putUrl, JSON.stringify(setting), HTTP_JSON_OPTIONS)
      .toPromise()
      .then(() => null)
      .catch(error => {
  ...
  resetPassword(uuid: string, newPassword: string): Promise<any> {
    ...
    return this.http
      .post(resetPasswordEndpoint, body.toString(), HTTP_FORM_OPTIONS)
      .toPromise()
      .then(response => response)
      .catch(error => {
        return Promise.reject(error);
